...
public class GlideCatchTool {
    private static GlideCatchTool instance;

    public static GlideCatchTool getInstance() {
        if (null == instance) {
            instance = new GlideCatchTool();
        }
        return instance;
    }
    ...
    public String getCacheSize() {
        try {
            return getFormatSize(getFolderSize(new File(VariableTool.IMAGE_SAVE_DIRECTORY)));
        } catch (Exception e) {
            e.printStackTrace();
            return "获取失败";
        }
    ...
    public boolean cleanCatchDisk() {
        return deleteFolderFile(VariableTool.IMAGE_SAVE_DIRECTORY, true);
    }
